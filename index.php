...
//取文件发布者、名称、大小、描述
preg_match('~<span class="p7">分享用户：</span><font>(.*?)</font><br>~', $softInfo, $softFileAuthor);
$softFileAuthor = isset($softFileAuthor[1]) ? $softFileAuthor[1] : "";

preg_match('~<span class="p7">运行系统：</span>(.*?)<br>~', $softInfo, $softFileSystem);
$softFileSystem = isset($softFileSystem[1]) ? $softFileSystem[1] : "";


preg_match('~<span class="p7">文件描述：</span><br>([\s\S]*?)</td>~', $softInfo, $softFileDesc);
if (isset($softFileDesc[1])) {
    // 假设 $softFileDesc[1] 包含匹配到的内容
    $rawContent = $softFileDesc[1];
    // 第一步：去除 HTML 标签
    $plainText = strip_tags($rawContent);
    // 第二步：解码 HTML 实体
    $decodedText = html_entity_decode($plainText, ENT_QUOTES, 'UTF-8');
    // 第三步：将 <br> 替换为换行符（如果需要）
    $cleanText = str_replace('<br>', "\n", $decodedText);
    // 第四步：去除多余的空白字符
    $softFileDesc = trim($cleanText);
}
    ...
    die(json_encode(
            array(
                ...
                'data' => array(
                    ...
                    'fileauthor' => $softFileAuthor,
                    'filesystem' => $softFileSystem,
                    'filesize' => isset($softFilesize[1]) ? $softFilesize[1] : "",
                    'filedesc' => isset($softFileDesc[1]) ? $softFileDesc : "",
                    'downUrl' => $downUrl
                )
            ),
            JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES
